...
class Generator:
    ...
    def transfer(self):
        for a in range(len(self.string_codedBits)):
            temp = ''
            for b in range(len(self.string_codedBits[a])):
                rand = random.randint(0,9)
                if rand == 0:
                    if self.string_codedBits[a][b] == '0':
                        temp += '1'
                    if self.string_codedBits[a][b] == '1':
                        temp += '0'
                else:
                    temp += self.string_codedBits[a][b]
            self.string_codedBits[a] = temp
            print(temp)
